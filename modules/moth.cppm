...
export {
    ...
    /* Abstract moth class, from which all the other classes inherit.
     * Provides feed_cycle() method, which is partly implemented here,
     * makes a moth move on a given string. Provides operator<<.
    */
    class AbstractMoth {
        protected:
            param_t _pos;
    ...
    class FussyMoth : public AbstractMoth {
        private:
            ...
            /* x must be neither letter nor number nor space to return true */
            bool can_consume(char x) const override {
                return !((x >= '0' && x <= '9') || 
                (x >= 'A' && x <= 'Z') || (x >= 'a' && x <= 'z') || x == ' ');
            }
